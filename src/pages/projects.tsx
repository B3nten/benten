...
export default function Projects(props: any) {
    return (
        <>
            <Head>
                <title>@benten: projects</title>
                <link rel="shortcut icon" href="/favicon.png" />
            </Head>
            <section className="section space-y-10">
                <h1 className='page-title'>Projects</h1>
                {props.posts.result.featured.map((p: any) => <FeaturedProject key={p.name} project={p} />)}
                <div className="grid md:grid-cols-2 gap-4">
                    {props.posts.result.projects.map((p: any) => <Project key={p.name} project={p} />)}
                </div>
            </section>
        </>
    )
...
function FeaturedProject(project: any) {
    ...
    return (
        <div className='block relative w-full p-4 md:p-6 bg-background-focus rounded-lg'>
            <img className='h-[496px] w-full object-cover rounded-md' src={project.project.image} />
            <div className='flex flex-col mt-4 md:flex-row justify-between'>
                ...
                <div className='flex space-x-3 mt-4 md:mt-0'>
                    {project.project.external &&
                        <a rel='noreferrer' target='_blank' href={project.project.external}>
                            <FiExternalLink className='w-10 h-10 transition hover:stroke-primary' />
                        </a>}
                    {project.project.github &&
                        <a rel='noreferrer' target='_blank' href={project.project.github}>
                            <SiGithub className='w-10 h-10 stroke-primary transition hover:fill-secondary' />
                        </a>}
                    {project.project.blog &&
                        <a rel='noreferrer' target='_blank' href={project.project.blog}>
